...
public class VoiceChat : NetworkBehaviour
{
	...
	void Target_PlaySound(NetworkConnection connection, byte[] DestBuffer, uint BytesWritten)
	{
		...
		Debug.Log((uint)DestBuffer2.Length);
		...
		if (ret == EVoiceResult.k_EVoiceResultOK && BytesWritten2 > 0)
		{
			...
			float[] datas = new float[22050];
			for (int i = 0; i < datas.Length; ++i)
			{
				datas[i] = (short)(DestBuffer2[i * 2] | DestBuffer2[i * 2 + 1] << 8) / 32768.0f;
			}
			

			audioSource.clip.SetData(datas, 0);
			audioSource.Play();
		}
	}
}